...
def default_reproduction_stages(*, include_qgis: bool) -> tuple[RunStage, ...]:
    ...
    stages = [
        ...
        RunStage("model refit", (python, "scripts/refit_extended_training_models.py"), "Refit the validation-selected final nine-feature model and validate only 2020-2021."),
        RunStage("final-model evidence", (python, "scripts/run_hyperparameter_experiments.py", "--full-training", "--run-name", "full_training_all_candidates"), "Reproduce validation-only parameter-selection evidence for the final model."),
        RunStage("final temporal test", (python, "scripts/run_extended_final_temporal_test.py"), "Evaluate the frozen final nine-feature model once on held-out T=2022-2024 labels; no tuning occurs."),
        RunStage("model diagnostics", (python, "scripts/build_model_diagnostics.py"), "Build final-test regression diagnostics from the frozen final-model evaluation."),
        ...
        RunStage("operational score", (python, "scripts/score_operational_forecast.py", "--replace-existing"), "Republish the target-free 2026 comparative estimate with the documented final-model artifact."),
        RunStage("local web-map asset", (python, "scripts/build_web_map_assets.py", "--overwrite"), "Create the reduced browser-ready 2026 map asset from the validated annual GeoPackage."),
        ...
        RunStage("final-model selection figures", (python, "scripts/build_model_v2_validation_figures.py"), "Build durable validation-only parameter-selection figures."),
...
def _output_inventory() -> list[dict[str, object]]:
    ...
    candidates = (
        ...
        ("2026 local web-map asset", PROJECT_ROOT / "data/processed/web_map/estimated_comparative_wildfire_exposure_2026.geojson"),
