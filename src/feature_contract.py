...
CLIMATE_PREDICTOR_COLUMNS = PREDICTOR_COLUMNS[4:]
...
def validate_feature_table(
    ...
    *,
    ...
    climate = list(CLIMATE_PREDICTOR_COLUMNS)
    climate_masks = table[climate].isna()
    if not climate_masks.eq(climate_masks.iloc[:, 0], axis=0).all().all():
        raise ValueError("ERA5-Land mask must be identical across all climate features")
