...
class WebMapTests(unittest.TestCase):
    ...
    def test_browser_client_has_documented_basemap_choices_and_full_view_shell(self) -> None:
        ...
        self.assertIn('title="Show the nine model inputs used for this selected cell"', page)
        self.assertIn('title="Clear the selected cell and nearby-context highlights"', page)
        self.assertIn('title="Close this explanation"', page)
        self.assertIn("toolbar-icon", page)
        self.assertIn("toolbar-icon", stylesheet)
        self.assertIn("input-details-dialog", stylesheet)
        self.assertIn("L.control.scale", client)
        self.assertIn("measurement-control", client)
        self.assertIn('position: "topright"', client)
        self.assertIn("installMeasurementControl", client)
        self.assertIn('data-measure="distance"', client)
        self.assertIn('data-measure="area"', client)
        self.assertIn('data-measure="clear"', client)
        self.assertIn("geodesicArea", client)
        self.assertIn("finishMeasurement", client)
        self.assertIn("clearMeasurements", client)
        self.assertIn('color: "#22d3ee"', client)
        self.assertIn("leaveMeasurementMode", client)
        self.assertIn("interactive: false", client)
        self.assertIn("map.dragging.enable()", client)
        self.assertIn('map.createPane("measurementPane")', client)
        self.assertIn('style.pointerEvents = "none"', client)
        self.assertIn("renderer: measurementRenderer", client)
        ...
        self.assertIn('id="legend-help-button"', page)
        self.assertIn('id="legend-help-popover"', page)
        self.assertIn("styles.css?v=20260817-legend-help", page)
        self.assertIn("app.js?v=20260817-legend-help", page)
        self.assertIn("89,112 grid cells", page)
        self.assertIn("what share of the cell may burn during the following year", page)
        self.assertIn("showLegendHelp", client)
        self.assertIn('addEventListener("mouseenter", showLegendHelp)', client)
        self.assertIn("legend-help-popover", stylesheet)
        self.assertIn("showFeatureHelp", client)
        self.assertIn("inputDetailsDialog.appendChild(featureHelpPopover)", client)
        self.assertIn("const preferredLeft", client)
        ...
        self.assertIn('selectionTitle.textContent = "Selected cell"', client)
        self.assertIn("selectionToolbar.hidden = true", client)
        self.assertIn("card-header", page)
        self.assertIn("card-header", stylesheet)
