...
class PublicReproducibilityTests(unittest.TestCase):
    ...
    def test_reproduction_stages_are_explicit_and_platform_neutral(self) -> None:
        ...
        self.assertLess(names.index("model refit"), names.index("final-model evidence"))
        self.assertLess(names.index("final-model evidence"), names.index("final temporal test"))
